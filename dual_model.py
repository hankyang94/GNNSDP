...
class DualModelFtype(nn.Module):
    def __init__(self, node_feature_mode=1,
                       ...
                       residual=True,
                       batchnorm=True):
        super(DualModelFtype,self).__init__()
    ...
    def post_mp_one_graph(self,x,ud_edges):
        ...
        for mlp_layer in self.dual_node_mlp[:-1]:
            vd = mlp_layer(vd)
            vd = F.leaky_relu(vd,negative_slope=self.relu_slope)
            vd = F.dropout(vd,p=self.dropout_rate,training=self.training)
        vd = self.dual_node_mlp[-1](vd)
        # Dual edge
        ...
        for mlp_layer in self.dual_edge_mlp[:-1]:
            ed = mlp_layer(ed)
            ed = F.leaky_relu(ed,negative_slope=self.relu_slope)
