...
def train(dataset,writer,batch_size,num_epoches):
    ...
    for epoch in range(num_epoches):
        ...
        for batch in loader:
            ...
            _, U = model(batch)
            dual_loss = model.loss(batch.S,U)
            dual_loss.backward()
            opt.step()
...
filename = f'./models/dual_factor_model_{GNN_TYPE}_{GNN_LAYER}_{GNN_HIDDEN_DIM}_{GNN_OUT_DIM}_{NODE_MODE}_{NUM_EPOCHES}_{MLP_LAYER}.pth'
